...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{>{\bfseries}L{3.2cm}L{2.5cm}L{2.5cm}Y}
    \toprule
    \tableheader \thead{Tác vụ} & \thead{Ngữ cảnh} & \thead{Chu kỳ/sự kiện} & \thead{Công việc chính} \\
    \midrule
    TIM6 callback & ISR & \SI{10}{\milli\second} & Tăng bộ đếm tick, không xử lý game. \\
    Game update & Main & \SI{20}{\milli\second} & Vật lý, va chạm, dựng frame và page flip. \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{>{\bfseries}L{2.7cm}L{3.4cm}L{2.7cm}Y}
    \toprule
    \tableheader \thead{Trạng thái} & \thead{Sự kiện} & \thead{Trạng thái kế} & \thead{Hành động} \\
    \midrule
    ...
    OVER/\allowbreak PAUSED & Menu & READY & Trở lại menu chính. \\
    READY & Bird/\allowbreak Difficulty/\allowbreak Sound & Màn cài đặt & Hiển thị và cập nhật tùy chọn tương ứng. \\
    \bottomrule
  \end{tabularx}
\end{table}
