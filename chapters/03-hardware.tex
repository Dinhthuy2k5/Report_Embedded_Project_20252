...
Hình~\ref{fig:system-block} thể hiện các khối chính và giao tiếp được dùng trong phiên bản mã nguồn hoàn chỉnh.
...
\begin{table}[H]
  ...
  \label{tab:pinout}
  \begin{tabularx}{\textwidth}{>{\bfseries}p{3.1cm}p{2.5cm}X}
    \toprule
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{>{\bfseries}p{3.0cm}p{3.2cm}X}
    \toprule
    \tableheader Khối nguồn & Khối đích & Vai trò trong hệ thống \\
    \midrule
    STMPE811 & CPU qua I2C3 & Cung cấp tọa độ chạm cho menu và lệnh nhảy. \\
    \rowcolor{TableAlt}CPU/DMA2D & SDRAM & Dựng khung hình mới trong back buffer. \\
    SDRAM & LTDC/LCD & LTDC đọc liên tục front buffer để quét RGB ra panel. \\
    \rowcolor{TableAlt}CPU & DMA1/I2S3 & Nạp mẫu nhạc và hiệu ứng vào buffer vòng. \\
